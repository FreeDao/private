  ...
  <div id="myBody">
   ...
   <ul class="breadcrumb" style="background-color: white;margin-bottom: 0;">
	  <li>首页</li>
                                ...
                                <c:forEach items="${pu.data }" var="pd" >
                                	...
                                	<tr class="${temp }" style="height: 50px;" >
	                                    ...
	                                    <td>
	                                        ${pd.dl }
	                                        <ul class="actions">
	                                        	<li class="last"><i class="icon-gift" onclick="give('${pd.id }')"></i></li>
	                                           <li class="last"><i class="table-delete" onclick="del('${pd.id }')"></i></li>
	                                        </ul>
	                                        <script type="text/javascript">
	                                        function give(id){
	                            				$('#giveIntegral').modal('show');
	                            				$("#integralUserId").val(id);
	                            				$("#integralQueryValue").val();
	                            				//$('#geiveSure').attr("href","bg/query!giveIntegral?userId="+id+"&integral="+$("#integral").val()+"&queryValue=${queryValue }");
	                                        }
	                                        function del(id){
	                            				$('#myModal').modal('show');
            ...
            <!-- Modal -->
            <form action="bg/query!giveIntegral">
			<div id="giveIntegral" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="width: auto;" >
			  
			  <input type="hidden" name="userId" id="integralUserId"/>
			  <input type="hidden" name="queryValue" id="integralQueryValue" value="${queryValue }"/>
			  <div class="modal-header">
			    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			    <h3 id="myModalLabel">赠送积分</h3>
			  </div>
			  <div class="modal-body" id="myModalBody">
			    	积分数量：<input id="integral" name="integral" style="width: 50px;" onkeyup="validNumber(this,true)"/>
			    	
			  </div>
			  <div class="modal-footer">
			  	<a class="btn-flat white" data-dismiss="modal" aria-hidden="true">关闭</a>
			  	<input id="geiveSure" type="submit" class="btn-flat primary" value="提交" />
            ...
            <script type="text/javascript" src="js/myVaild.js"></script>
    </div>
  </body>
</html>
