...
//		  System.out.println(targetURL);
		  ...
		  if (!"/bg/user!bg".equals(targetURL) && !"/bg/user!login".equals(targetURL)) {//判断当前页是否是重定向以后的登录页面页面，如果是就不做session的判断，防止出现死循�?
		   if (session == null || session.getAttribute("userSession") == null) {//*用户登录以后�?��动添加session
		    request.setAttribute("msg", "请先登陆");
		    response.sendRedirect("/fp/bg/user!bg");//如果session为空表示用户没有登录就重定向到login.jsp页面
		    return;
		   }
		  }
		  //加入filter链继续向下执�?
