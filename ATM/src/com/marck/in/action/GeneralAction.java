...
public class GeneralAction extends BaseAction {
	...
	public String apply(){
		try {
			...
			if( apply == null || CommonUtil.validParams(apply.getUserId(),apply.getNum(),apply.getType()) ){
				map.put("msg", "缺少必传参数");
				map.put("code", 0);
				return SUCCESS;
			}
	...
	public String bindAlipay(){
		try {
			...
			if( CommonUtil.validParams(userId,alipay) ){
				map.put("msg", "缺少必传参数");
				map.put("code", 0);
				return SUCCESS;
			}
			
			generalService.bindUserAlipay(userId,alipay,phone,name,map);
	...
	public String selUserAlipay(){
		try {
			...
			generalService.selUserAlipay(userId,map);
	...
	*/
	...
	public String dm(){
		try {
			...
			generalService.addIntergral(userId,openudid,points,adname,DateUtil.getNowString("yyyy-MM-dd HH:mm:ss"),orderId,price,9,map);
