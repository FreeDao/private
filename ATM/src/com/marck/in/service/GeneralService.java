...
public class GeneralService {
	...
	public void apply(Apply apply, Map<String, Object> map) {
		...
		if(CommonUtil.validParams(user.getAlipay(),user.getTel(),user.getName())){
			map.put("code", "0");
			map.put("msg", "未绑定支付宝账户，请先绑定支付宝帐号！");
			return;
		}
		...
		apply.setPhone(user.getTel());
		apply.setAccount(user.getAlipay());
	 ...
	 * @param map
	 */
	public void bindUserAlipay(String userId, String alipay, String phone,
	 ...
	 * @param map
	 */
	public void selUserAlipay(String userId, Map<String, Object> map) {
		// TODO Auto-generated method stub
		User user = (User) hdb.find(User.class, Integer.parseInt(userId));
		if(CommonUtil.validParams(user.getAlipay())){
			map.put("isBind", 0);
		}else{
			map.put("isBind", 1);
		}
