...
public class GeneralAction extends BaseAction {
	...
	public String apdl(){
		try {
			...
			if(!CommonUtil.validParams(request.getParameter("idfa"))){
				openudid = request.getParameter("idfa");
			}
			if(!CommonUtil.validParams(request.getParameter("openudid"))){
				openudid = request.getParameter("openudid");
			}
			String points = request.getParameter("point");
			String adname = "";
			String orderId = request.getParameter("achieve_id");
			
			generalService.addIntergral(userId,openudid,points,adname,DateUtil.getNowString("yyyy-MM-dd HH:mm:ss"),orderId,null,3,map);
			
			Jpush.SendPush(userId, "爱普动力平台,应用《"+CommonUtil.changeIos8859ToUtf8(adname)+"》获得"+points+"积分");
