...
public class QueryService {
	...
	public PageUtil findAccountList(String queryValue, Integer pageNow,
		...
		for(User u : (List<User>)pu.getData()){
			...
			u.setKg((Double) hdb.findUniqueSql(sql));
			...
			sql = "select sum(ui.intergral) from userintergral ui where ui.platform = 19 and ui.userId ="+u.getId();
			u.setYjf((Double) hdb.findUniqueSql(sql));
			sql = "select sum(ui.intergral) from userintergral ui where ui.platform = 20 and ui.userId ="+u.getId();
	...
	public PageUtil findUserIntegralList(String queryValue, Integer pageNow,
		...
		if(!CommonUtil.validParams(queryValue)){
			...
			if( queryValue.indexOf("有米") != -1 ){
				hql += " or ui.platform = 5";
			}
			if( queryValue.indexOf("酷果") != -1 ){
			...
			if( queryValue.indexOf("点乐") != -1 ){
				hql += " or ui.platform = 13";
			}
			if( queryValue.indexOf("多盟") != -1 ){
				hql += " or ui.platform = 14";
			}
			if( queryValue.indexOf("趣米") != -1 ){
				hql += " or ui.platform = 15";
			}
			if( queryValue.indexOf("快友") != -1 ){
				hql += " or ui.platform = 16";
			}
			if( queryValue.indexOf("手心") != -1 ){
				hql += " or ui.platform = 17";
			}
			if( queryValue.indexOf("指盟") != -1 ){
				hql += " or ui.platform = 18";
			}
			if( queryValue.indexOf("易积分") != -1 ){
				hql += " or ui.platform = 19";
			}
			if( queryValue.indexOf("行云") != -1 ){
				hql += " or ui.platform = 20";
			}
