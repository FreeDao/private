	  ...
	  <li class="active">用户管理</li>
                            ...
                            <tbody>
                                ...
                                <c:forEach items="${pu.data }" var="pd" >
                                	...
                                	<tr class="${temp }" style="height: 50px;" >
	                                    ...
	                                    <td >
	                                    	<c:if test="${fn: length(pd.address)>51}">  
									          	${fn: substring(pd.address, 0, 52)}...
									          </c:if> <c:if test="${fn: length(pd.address)<=51}">
									          	${pd.address}
									          </c:if>
	                                    </td>
	                                    <td >
	                                        ${pd.addTime }
	                                        <ul class="actions">
	                                            <li><a href="bg/system!goAddOrUpdateAccount?user.id=${pd.id }"><i class="table-edit" ></i></a></li>
	                                        ...
	                                        function del(id){
	                            				...
	                            				$('#sure').attr("href","bg/system!delAccount?user.id="+id+"&queryValue=${queryValue }");
	                                        }
	                                        </script>
	                                    </td>
	                                </tr>
			...
			<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			  ...
			  <div class="modal-body">
			    <p id="tipContent">确定删除此用户？若删除该用户的相关数据将一并删除，比如：该用户关联商铺，商品，评论信息，是否确认？</p>
			  </div>
